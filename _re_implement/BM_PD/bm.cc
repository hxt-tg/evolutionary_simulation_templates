...
#ifdef GRID_NET
#define L             10           /* Side length of a network */
...
#define MC_STEPS      100           /* Number of MC steps */
#define AVG_STEPS     1             /* Number of steps at the end for averaging */
#define REFRESH_FREQ  5             /* Control the frequency of refresh screen, 1 for all the time */
#define TRY_TIMES     100           /* Average result in tried times */
/* Running parameters */
double A_start = -1, A_end = 3.5, A_interval = 0.05;
double beta_start = 0, beta_end = 2, beta_interval = 0.05;
/* =================== End of settings =================== */
...
void save_meta() {
    ...
    fprintf(fp, "A=%g~%g with interval %g.\n", A_start, A_end, A_interval);
    fprintf(fp, "beta=%g~%g with interval %g.\n", beta_start, beta_end, beta_interval);
#endif
    fclose(fp);
}
...
/* Call update_matrix(b) after loop for b (DEPRECATED)
...
void init_randomly(PDNetwork &net) {
    for (int i = 0; i < N_STRA; i++) stra_cnt[i] = 0;
    for (int i = 0; i < SIZE; i++) net[i].p = 0.5;
}
...
void init_by_st_mat(PDNetwork &net) {
#ifndef GRID_NET
...
void update_p(PDNetwork &net) {
    for (int &x : net.nodes()) {
        double s = tanh(beta * (net[x].payoff - A));
        if (net[x].stra == _C && s >= 0) 
            net[x].p = net[x].p + (1-net[x].p) * s;
        else if (net[x].stra == _C && s < 0)
            net[x].p = net[x].p + net[x].p * s;
        else if (net[x].stra == _D && s >= 0)
            net[x].p = net[x].p - net[x].p * s;
        else // (net[x].stra == _D && s < 0)
            net[x].p = net[x].p - (1-net[x].p) * s;
    }
}

void update_stra(PDNetwork &net) {
    for (int i = 0; i < N_STRA; i++) stra_cnt[i] = 0;
    for (int &x : net.nodes())
        stra_cnt[net[x].stra = randf() < (net[x].p*(1-EPSILON)+(1-net[x].p)*EPSILON) ? _C : _D]++;
}

void update_payoff(PDNetwork &net) {
    for (int &x : net.nodes()) {
        net[x].payoff = 0;
        if (net.degree(x) == 0) continue;
        for (auto &n : net.neighbors(x))
            net[x].payoff += payoff_matrix[net[x].stra][net[n].stra];
        net[x].payoff /= net.degree(x);
    }
}


/* Snapshot function temporarily not used */
void matrix_snapshot(PDNetwork &net, int try_time, int step) {
#ifndef REC_SNAPSHOT
    return ;
#endif
#ifndef GRID_NET
    sprintf(stderr, "Matrix snapshot is only supported on GridNetwork.\n");
    exit(1);
#endif
    if (TRY_TIMES > 1) sprintf(_file_path, "output/snapshots/beta=%.2lf_A=%.2lf_step=%d[%d].csv", beta, A, step+1, try_time);
    else sprintf(_file_path, "output/snapshots/beta=%.2lf_A=%.2lf_step=%d.csv", beta, A, step+1);
...
void run(PDNetwork &net, FILE *fp_aver) {
    FILE *fp_steps = NULL, *fp_p = NULL;
#ifdef REC_STEPS
    sprintf(_file_path, "output/steps/beta=%.2lf_A=%.2lf.csv", beta, A);
    fp_steps = fopen(_file_path, "w");
    if (TRY_TIMES > 1) fprintf(fp_steps, "try,");
    fprintf(fp_steps, "step,f_C,f_D\n");
    sprintf(_file_path, "output/p/beta=%.2lf_A=%.2lf.csv", beta, A);
    fp_p = fopen(_file_path, "w");
    if (TRY_TIMES > 1) fprintf(fp_p, "try,");
    fprintf(fp_p, "step,f_C,f_D\n");
#endif
    ...
    for (try_time = 0; try_time < TRY_TIMES; try_time ++) {
        ...
        init_randomly(net);
#endif
        matrix_snapshot(net, try_time, -1);
        for (step = 0; step < MC_STEPS; step++) {
            ...
            for (auto &x: net.nodes())
                avg_p += (net[x].p*(1-EPSILON)+(1-net[x].p)*EPSILON);
            avg_p /= net.number_of_nodes();

            if (TRY_TIMES > 1) fprintf(fp_p, "%d,", try_time);
            fprintf(fp_p, "%d,%lf,%lf\n", step, avg_p, 1-avg_p);
            // End of Recording
        ...
        printf(" beta: %.2lf   A: %.2lf   avg: [%6.2lf%%,%6.2lf%%]       \n", beta, A, 
    ...
    fprintf(fp_aver, "%.2lf,%.2lf,%lf,%lf\n", beta, A, aaf[_C], aaf[_D]);
    if (TRY_TIMES > 1) printf("[%d tries] beta: %g  A: %g  avg: [%6.2lf%%,%6.2lf%%]\n\n",
            TRY_TIMES, beta, A, aaf[_C]*100, aaf[_D]*100);
}
...
int main() {
    ...
    fprintf(fp_aver, "beta,A,f_C,f_D\n");
...
#if defined(GRID_NET)
    GridNetwork<NodeData, EdgeData> net(L, L, N_NEIGH);
#else
    ...
    for (param_idx = 0; param_idx < sizeof(SPEC_PARAM_LIST)/(sizeof(double) * N_PARAM); param_idx ++) {
        beta = SPEC_PARAM_LIST[param_idx][0];
        A = SPEC_PARAM_LIST[param_idx][1];
        run(net, fp_aver);
    }
#else
    for (beta = beta_start; beta < beta_end || fabs(beta - beta_end) < 1e-7; beta += beta_interval)
    for (A = A_start; A < A_end || fabs(A - A_end) < 1e-7; A += A_interval)
        run(net, fp_aver);
#endif
    fclose(fp_aver);
    return 0;
