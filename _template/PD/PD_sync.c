...
Strategy stra[SIZE], last_stra[SIZE];
...
void init_randomly() {
    ...
    for (i = 1; i < SIZE; i++) stra_cnt[stra[i] = (Strategy)randi(N_STRA)]++;
}
...
void calc_last_payoff_by_group() {
    int x;
    for (x = 0; x < SIZE; x++) last_payoff[x] = 0;
    for (x = 0; x < SIZE; x++) {
        last_payoff[x] += payoff_matrix[stra[x]][stra[net[x][0]]];
        last_payoff[x] += payoff_matrix[stra[x]][stra[net[x][1]]];
        last_payoff[net[x][0]] += payoff_matrix[stra[net[x][0]]][stra[x]];
        last_payoff[net[x][1]] += payoff_matrix[stra[net[x][1]]][stra[x]];
    }
}
