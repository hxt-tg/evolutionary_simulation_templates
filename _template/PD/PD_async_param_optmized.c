...
const struct option long_options[] = {
    ...
    {"steps-dir",    required_argument, 0, 's' },
    {"avg-file",     required_argument, 0, 'a' },
    {"random-seed",  required_argument, 0, 'r' },
    {"stra-name",    required_argument, 0, 'S' },
...
printf(" [EXAMPLE]   .\\%s -b1,1.2,0.05 --steps-dir* --avg-file* (No file output)\n", progname);
}
...
void read_parameters(int argc, char **argv) {
    ...
    while (1) {
        int option_index = 0;
        c = getopt_long(argc, argv, "d:s:a:r:S:L:K:T:M:R:b:F:h", long_options, &option_index);
        if (c == -1) break;
        switch (c) {
        case 'd':
...
void save_metadata() {
    ...
    make_dir(_file_path);
    sprintf(_file_path, "%s/%s.csv", OUTPUT_DIR, META_FN);
    ...
    fprintf(fp, "STEPS_FILE,\"%s/%s.csv\"\nAVG_FILE,\"%s/%s.csv\"\n", 
        OUTPUT_DIR, AVG_FN, OUTPUT_DIR, STEPS_DIR);
    fclose(fp);
}
...
void snapshot(int step) {
    ...
    make_dir(_file_path);
    sprintf(_file_path, "%s/%s/step=%d.csv", OUTPUT_DIR, SNAPSHOT_DIR, step+1);
    FILE *fp = fopen(_file_path, "w");
    if (!fp) die(ENOENT, "Cannot create file for snapshot.");
    for (x = 0; x < SIZE; x++)
        fprintf(fp, "%c%s", STRA_STR[stra[x]], (x+1)%L?",":"\n");
    fclose(fp);
}
...
void run() {
    ...
    if (AVG_FN[0]) {
        sprintf(_file_path, "%s/%s_b=%g-%g.csv", OUTPUT_DIR, AVG_FN, bs, be);
    ...
    for (b = bs; b < be || fabs(b-be) < 1e-7; b += bi) {
        if (STEPS_DIR[0]) {
            sprintf(_file_path, "%s/%s/b=%g.csv", OUTPUT_DIR, STEPS_DIR, b);
        ...
        for (try_time = 0; try_time < TRY_TIMES; try_time ++) {
            ...
            for (step = 0; step < MC_STEPS; step++) {
                ...
                if (step > MC_STEPS-REC_STEPS-1) tac[try_time] += stra_cnt[0], rec++;
                
                if (STEPS_DIR[0]) {
                    if (TRY_TIMES > 1) fprintf(fp_steps, "%d,", try_time);
                	fprintf(fp_steps, "%d,%g\n", step, fc);
                }
        ...
        for (x = 0; x < TRY_TIMES; x ++)
            aafc += afc[x];
        aafc /= TRY_TIMES;
        if (AVG_FN[0]) fprintf(fp_avg, "%g,%g\n", b, aafc);
