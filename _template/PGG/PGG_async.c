...
/* ====================== Settings ======================= */
  /* Global settings */
#define REC_SNAPSHOT  0             /* Not euqal 0 if record snapshots */
...
#define N_PI_NEIGH    12            /* Number of neighbors for every node while updating pi */
  /* Monte Carlo settings */
#define K             0.5           /* Fermi coefficient */
#define MC_STEPS      20000         /* Number of MC steps */
#define AVG_STEPS     1000          /* Number of steps at the end for averaging */
...
#define TRY_TIMES     3             /* Average result in tried times */
  /* Running parameters */
double r_start = 3.5, r_end = 4, r_interval = 0.1;
/* =================== End of settings =================== */
...
void init_randomly() {
    ...
    for (i = 0; i < SIZE; i++) {
        stra_cnt[stra[i] = randi(N_STRA)]++;
    }
}

double pool(int x) {
    // 1. x as group id;
    // 2. Return value is multiplied by multiplier r.
    double _pool = stra[x] != _D ? cost : 0;
    int i;
    for (i = 0; i < N_NEIGH; i++)
        _pool += stra[net[x][i]] != _D ? cost : 0;
    return _pool * r;
}

double payoff_from_group(int x, int group) {
    double _pool_payoff = pool(group)/(N_NEIGH+1);
    if (stra[x] != _D) 
        _pool_payoff -= cost;
    return _pool_payoff;
}
...
double payoff(int x) {
    double gain = payoff_from_group(x, x);
    int i;
    for (i = 0; i < N_NEIGH; i++)
        gain += payoff_from_group(x, net[x][i]);
    return gain;
}
